...
(function () {
  ...
  // -------------------------------------------------------------- constants
  var MAX_GRADE = 20; // grades are out of 20
  var GRADE = { EXCELLENT: 16, VERY_GOOD: 14, GOOD: 12, PASS: 10 }; // colour bands
  var CACHE_TTL_MS = 3 * 24 * 60 * 60 * 1000; // obligation cache lifetime
  var OBLIGATIONS_PAGE_SIZE = 2000; // /api/obligations page size
  var OBLIGATIONS_MAX_PAGES = 4; // safety cap on pagination
  ...
  var S = (function () {
    try {
      return Object.assign({}, DEFAULTS, JSON.parse(localStorage.getItem("capella.settings") || "{}"));
    } catch (e) {
  ...
  function saveSettings() {
    try {
      localStorage.setItem("capella.settings", JSON.stringify(S));
    } catch (e) {}
  }
  ...
  function gradeColor(v) {
    ...
    if (n >= GRADE.EXCELLENT) return "#1a7f45";
    if (n >= GRADE.VERY_GOOD) return "#2d8f6f";
    if (n >= GRADE.GOOD) return "#2b6cb0";
    if (n >= GRADE.PASS) return "#c07a1e";
    return "#c0392b";
  }
  ...
  function moduleStatus(v) {
    ...
    if (n < GRADE.PASS) return { t: "à rattraper", c: "orange" };
  ...
  function gradeTint(v) {
    ...
    if (n >= GRADE.EXCELLENT) return "#e6f4ec";
    if (n >= GRADE.VERY_GOOD) return "#e7f2ee";
    if (n >= GRADE.GOOD) return "#e8f0f9";
    if (n >= GRADE.PASS) return "#faf1e2";
    return "#f9e9e7";
  }
  ...
  function classifyResult(resp) {
    ...
    var hasCoef = leaves.some(function (l) {
      return l.field === "obligationRelationParentCoefficient";
    });
    var hasCaption = leaves.some(function (l) {
      return l.field === "caption";
    });
    var hasCode = leaves.some(function (l) {
      return l.field === "code";
    });
  ...
  function readObCache() {
    try {
      ...
      if (o && o.ob && Date.now() - o.t < CACHE_TTL_MS) return o.ob;
    } catch (e) {}
    return null;
  }
  function writeObCache() {
    try {
      localStorage.setItem("capella.ob", JSON.stringify({ t: Date.now(), ob: OB }));
    } catch (e) {}
  }
  function useOb(ob) {
    OB = ob;
  ...
  async function loadCoefficients() {
    ...
    try {
      for (var page = 1; page <= OBLIGATIONS_MAX_PAGES; page++) {
        var r = await fetch("/api/obligations?size=" + OBLIGATIONS_PAGE_SIZE + "&page=" + page, {
      ...
      console.warn("[Capella] obligations fetch failed", e);
      obLoading = false;
      return false;
    }
    obLoading = false;
    if (Object.keys(ob.parent).length) {
      useOb(ob);
      writeObCache();
      return true;
    }
  ...
  function readIdentity(model) {
    ...
    if (gp && gp.json && gp.json.period && gp.json.period.caption) {
      model.year = nameOf(gp.json.period.caption)
        .replace(/Ann[ée]e acad[ée]mique/i, "")
        .trim();
    }
  }

  // Read the "synthesis" result: structure rows (with averages) + the newest
  // 4-digit period. Column positions are resolved from the metadata, not hardcoded.
  function readSynthesisRows(syn) {
    var out = { rows: [], period: "" };
    if (!syn || !syn.json || !syn.json.content) return out;
    var leaves = flattenColumns(syn.json.content.columns);
    var codeIdx = leaves.findIndex(function (l) {
      return l.field === "code";
    });
    var capIdx = leaves.findIndex(function (l) {
      return l.field === "caption";
    });
    var finalIdx = leaves.findIndex(function (l) {
      return /finale/i.test(labFr(l));
    });
    var beforeIdx = leaves.findIndex(function (l) {
      return /avant/i.test(labFr(l));
    });
    (syn.json.content.lines || []).forEach(function (line) {
      var code = line[codeIdx];
      if (!code) return;
      var m = /^(\d{4})_/.exec(code);
      if (m && (!out.period || m[1] > out.period)) out.period = m[1];
      var hasFinal = finalIdx >= 0 && line[finalIdx] != null;
      out.rows.push({
        code: code,
        name: nameOf(line[capIdx]),
        value: hasFinal ? line[finalIdx] : beforeIdx >= 0 ? line[beforeIdx] : null,
        kind: hasFinal ? "finale" : "provisoire",
      });
    });
    return out;
  }
  ...
  // tree knows it; otherwise it's an exam-level row. Fall back to the code pattern
  // only until obligations load.
  function isStructure(code) {
    if (OB.loaded) return !!(OB.parent[code] || OB.children[code]);
    return /_FISA_S\d/.test(code);
  }

  // Read the "exams" result: one entry per exam with grade, coefficient and type.
  function readExamRows(ev, examCaptions) {
    var exams = [];
    if (!ev || !ev.json || !ev.json.content) return exams;
    var leaves = flattenColumns(ev.json.content.columns);
    var markIdx = leaves.findIndex(function (l) {
      return l.field === "calculatedField" && /note|mark/i.test(labFr(l));
    });
    var coefIdx = leaves.findIndex(function (l) {
      return l.field === "obligationRelationParentCoefficient";
    });
    var codeIdx = leaves.findIndex(function (l) {
      return l.field === "code" && !/type/i.test(labFr(l));
    });
    var typeIdx = leaves.findIndex(function (l) {
      return l.field === "code" && /type/i.test(labFr(l));
    });
  ...
  // Parent of a node: authoritative from the obligation tree, else the longest
  // other code that is a prefix of this one.
  function parentOf(node, index) {
    if (OB.loaded) {
      var p = OB.parent[node.code];
      return p && index[p] ? index[p] : null;
    }
    var best = -1;
    var parent = null;
    Object.keys(index).forEach(function (code) {
      var o = index[code];
      if (o !== node && node.code.indexOf(o.code + "_") === 0 && o.code.length > best) {
        parent = o;
        best = o.code.length;
      }
    });
  ...
  var byCodeAsc = function (a, b) {
    return a.code < b.code ? -1 : 1;
  };

  // Build the semester → UE → module forest and attach exams to leaf modules.
  function buildSemesters(treeRows, period, exams) {
    var byCode = {};
    treeRows.forEach(function (r) {
      if (period && r.code.indexOf(period + "_") !== 0) return; // current year only
    ...
    var nodes = Object.keys(byCode).map(function (c) {
      var n = {
        code: c,
        name: OB.caption[c] || byCode[c].name,
    ...
    nodes.forEach(function (n) {
      var parent = parentOf(n, index);
      if (parent) parent.children.push(n);
      else roots.push(n);
    });
    var modByKey = {};
    nodes.forEach(function (n) {
      modByKey[normCode(n.code)] = n;
    });
    ...
    nodes.forEach(function (n) {
      n.children.sort(byCodeAsc);
      n.exams.sort(byCodeAsc);
    });
  ...
  function parse() {
    ...
    var syn = readSynthesisRows(findResult("synthesis"));
    var examCaptions = {};
    var treeRows = [];
    ...
    var exams = readExamRows(findResult("exams"), examCaptions);
    model.hasExams = exams.length > 0;
    if (!model.hasTree) return model;

    model.semesters = buildSemesters(treeRows, syn.period, exams);
    if (!model.year && syn.period) model.year = "20" + syn.period.slice(0, 2) + "-20" + syn.period.slice(2);
    return model;
  }
  ...
  function displayValue(node) {
    return simMode ? effectiveValue(node) : toNum(node.value);
  }
  ...
  function semesterEcts(sem) {
    var total = sem.children.reduce(function (s, c) {
      return s + (toNum(c.coef) || 0);
    }, 0);
  ...
  function simInput(node) {
    var inp = el("input", {
      ...
      step: "0.5",
      min: "0",
      max: String(MAX_GRADE),
      placeholder: node.value != null ? fmt(node.value) : "—",
      value: SIM[node.code] != null ? SIM[node.code] : "",
    });
    inp.oninput = function () {
      var v = inp.value.trim();
      if (v === "") delete SIM[node.code];
      else SIM[node.code] = v;
      recompute();
    };
    return inp;
  }

  // Semester card (depth 0).
  function renderSemester(node) {
    ...
    titles.appendChild(
      el("div", {
        class: "ap-sem-sub",
        text: node.kind === "finale" ? "Moyenne finale" : "Moyenne provisoire (semestre en cours)",
      })
    );
    ...
    if (ects.total) {
      var ectsBox = el("div", {
        class: "ap-sem-ects",
        text: "ECTS : " + ects.acquired + " / " + ects.total + " validés",
      });
    ...
    node.children.forEach(function (c) {
      body.appendChild(renderNode(c, 1));
    });
    card.appendChild(body);
    return card;
  }

  // One UE/module row (name, coef/ECTS, status, optional sim input, grade pill).
  function renderNodeRow(node, depth) {
    var row = el("div", { class: "ap-node-row" });
    var caret = el("span", { class: "ap-caret", text: node.children.length || node.exams.length ? "▾" : "" });
    ...
    if (node.coef != null)
      row.appendChild(
        el("span", { class: "ap-coef", text: isUE ? fmt(node.coef) + " ECTS" : "coef " + fmt(node.coef) })
      );
    ...
    if (simMode && !node.children.length) row.appendChild(simInput(node));
  ...
  function buildGradesPanel(model) {
    ...
    if (!model.hasTree) {
      ...
      empty.appendChild(
        el("p", {
          text: "Notes pas encore chargées. Ouvre « Mes notes (éval) » puis « Mes notes (synthèse) » dans le menu Auriga — Capella les affichera automatiquement.",
        })
      );
    ...
    if (simMode) {
      ...
      simbar.appendChild(
        el("span", {
          text: "🎯 Simulation : saisis des notes hypothétiques dans les modules ; les moyennes UE, semestre et ECTS se recalculent en direct.",
        })
      );
    ...
    model.semesters.forEach(function (s) {
      panel.appendChild(renderNode(s, 0));
    });
    ...
    glHead.onclick = function () {
      glBody.style.display = glBody.style.display === "none" ? "" : "none";
    };
  ...
  function build(model) {
    ...
    brand.appendChild(
      el("span", { class: "ap-sub", text: (model.name || "") + (model.year ? " · " + model.year : "") })
    );
    header.appendChild(brand);
    header.appendChild(el("div", { class: "ap-spacer" }));
    if (model.hasTree) {
      var simBtn = el("button", {
        class: simMode ? "ap-btn-on" : "",
        text: simMode ? "🎯 Simulation activée" : "🎯 Simuler",
      });
  ...
  function parseSafe() {
    try {
      ...
      console.warn("[Capella] parse error", e);
      return { hasTree: false, hasExams: false, semesters: [], name: "", year: "" };
    }
  }
  ...
  function rerender() {
    try {
      ...
      console.warn("[Capella] render error", e);
    }
  }
  ...
  function ensureCoefs() {
    loadCoefficients().then(function (changed) {
      if (changed && root) rerender();
    });
  }
  ...
  try {
    ...
    // Load a cached obligation tree synchronously so coefficients/ECTS show on the
    // first render; a fresh copy is fetched in the background by ensureCoefs().
    var cachedOb = readObCache();
    if (cachedOb) useOb(cachedOb);
    build(parseSafe());
